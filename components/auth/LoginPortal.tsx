...
const LoginPortal: React.FC<LoginPortalProps> = ({ settings, facilitators, processedStudents, onLoginSuccess, onSuperAdminLogin, onFacilitatorLogin, onPupilLogin, onSwitchToRegister }) => {
  ...
  const [showPupilKey, setShowPupilKey] = useState(false);
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <input type="text" value={credentials.schoolName} onChange={(e) => setCredentials({...credentials, schoolName: e.target.value})} className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-5 py-4 text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/10 uppercase transition-all" placeholder="UNITED BAYLOR ACADEMY..." />
            </div>
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="text" value={credentials.schoolNumber} onChange={(e) => setCredentials({...credentials, schoolNumber: e.target.value})} className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-5 py-4 text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/10 uppercase transition-all" placeholder="UBA-2025-XXX" />
                  </div>
                  <div className="space-y-1">
            ...
            {authMode === 'PUPIL' && (
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-blue-900 uppercase tracking-widest ml-1">Academy Hub ID</label>
                    <input type="text" value={credentials.schoolNumber} onChange={(e) => setCredentials({...credentials, schoolNumber: e.target.value})} className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-5 py-4 text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/10 uppercase transition-all" placeholder="UBA-2025-..." />
                  </div>
                  <div className="space-y-1 relative">
                    <label className="text-[9px] font-black text-blue-900 uppercase tracking-widest ml-1">Index Number</label>
                    <div className="relative">
                      <input 
                        type={showPupilKey ? "text" : "password"} 
                        value={credentials.pupilIndex} 
                        onChange={(e) => setCredentials({...credentials, pupilIndex: e.target.value})} 
                        className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-5 py-4 text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/10 uppercase transition-all pr-12" 
                        placeholder="101..." 
                      />
                      <button 
                        type="button"
                        onClick={() => setShowPupilKey(!showPupilKey)}
           ...
           <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl relative border border-slate-200 animate-in zoom-in-95 duration-300">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-center bg-gray-50 px-6 py-4 rounded-2xl border border-slate-200">
